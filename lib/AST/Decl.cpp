...
bool ValueDecl::isInterfacePackageEffectivelyPublic() const {
  ...
  // attribute, and is defined in an interface module (or built from
  // interface), it can be referenced by another module that imports
  // it even though the defining interface module does not have
  // package-name (public/private interface); in such case, the decl
  // is treated as public and access checks in sema are skipped.
